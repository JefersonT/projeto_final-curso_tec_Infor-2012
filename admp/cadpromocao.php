...
if($_GET['funcao'] == "cadastrar" && $enviar_arquivo == "sim" && is_file($_FILES['arquivo']['tmp_name']) && $produto != "" && $marca !="" && $precoa !="" && $precod !="" && $senhal !=""){
	...
	if(!eregi("^image\/(jpeg|png|gif|pjpeg|jpg)$", $_FILES['arquivo']['type'])){
		...
		";
			...
			$sql = mysql_query("insert into cadastrar_promocao (foto, produto, marca, precoa, precod, peso) values ('$foto', '$produto', '$marca', '$precoa', '$precod', '$peso')");
