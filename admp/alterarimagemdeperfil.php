...
if($_GET['funcao'] == "alterarimagem"  && $enviar_arquivo == "sim" && is_file($_FILES['arquivo']['tmp_name']) && $senha !=""){
	...
	if(!eregi("^image\/(jpeg|png|gif|pjpeg|jpg)$", $_FILES['arquivo']['type'])){
		echo"
		<META HTTP-EQUIV=REFRESH CONTENT='O; URL=configuracaodeconta.php'>
		...
		";
			...
			$sql = mysql_query("UPDATE admp SET foto='$foto' WHERE id='$ids'");
	...
	echo"
					<META HTTP-EQUIV=REFRESH CONTENT='O; URL=configuracaodeconta.php'>
					<script type=\"text/javascript\">
					alert(\"Erro ao Alterar Imagem! Por Favor, Verifique Se Você Preencheu Todos Os Campo, E Tente Novamente.\");
					</script>
					";
	}
