...
if($_GET['funcao'] == "alterarimagem"  && $enviar_arquivo == "sim" && is_file($_FILES['arquivo']['tmp_name']) && $senha !=""){
	...
	if(!eregi("^image\/(jpeg|png|gif|pjpeg|jpg|JPG)$", $_FILES['arquivo']['type'])){
		echo"
		...
		";
			...
			$sql = mysql_query("UPDATE anuncios SET foto='$foto' WHERE id_a='$ids'");
	...
	echo"
					<META HTTP-EQUIV=REFRESH CONTENT='O; URL=anuncios.php'>
					<script type=\"text/javascript\">
					alert(\"Erro ao Alterar a Imagem! Por Favor, Verifique Se Você Preencheu Todos Os Campo, E Tente Novamente.\");
					</script>
					";
