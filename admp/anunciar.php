...
if($_GET['funcao'] == "anunciar" && $enviar_arquivo == "sim" && is_file($_FILES['arquivo']['tmp_name']) && $titulo != "" && $anuncio !="" && $senha !=""){
	...
	if(!eregi("^image\/(jpeg|png|gif|pjpeg|jpg)$", $_FILES['arquivo']['type'])){
		...
		";
			...
			if(file_exists("../adm/fotos/$foto")){
				$a = 1;
				while(file_exists("../adm/fotos/[$a]$foto")){
					$a++;
					}
				...
				if(!move_uploaded_file($_FILES['arquivo']['tmp_name'], "../adm/fotos/".$foto)){
					echo"
					<META HTTP-EQUIV=REFRESH CONTENT='O; URL=anuncios.php'>
					<script type=\"text/javascript\">
			...
			$sql = mysql_query("insert into anuncios (foto, titulo, texto) values ('$foto', '$titulo', '$anuncio')");
	...
	if($_GET['funcao'] == "anunciar" && $enviar_arquivo == "nao" && $titulo != "" && $anuncio !="" && $senha !=""){
		
		if( $senha == $senhal){
			
			$sql = mysql_query("insert into anuncios (titulo, texto) values ('$titulo', '$anuncio')");
	...
	echo"
					<META HTTP-EQUIV=REFRESH CONTENT='O; URL=anuncios.php'>
					<script type=\"text/javascript\">
					alert(\"Erro ao Postar Anúncios! Por Favor, Verifique Se Você Preencheu Todos Os Campo (Exeto o imagem, pois não é obrigatorio) E Tente Novamente.\");
					</script>
					";
	}
	}
